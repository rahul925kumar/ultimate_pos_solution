...
class Util
{
    ...
    public function num_f($input_number, $add_symbol = false, $business_details = null, $is_quantity = false)
    {
        ...
        if (!is_numeric($input_number)) {
            $input_number = 0.0; // Default to 0.0 if invalid
        } else {
            $input_number = (float) $input_number; // Convert to float
        }
    ...
    {
        // Validate inputs
        if (!is_numeric($number) || !is_numeric($percent) || !is_numeric($addition)) {
            return 0.0; // Return 0.0 if any input is not numeric
        }
